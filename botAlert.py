...
result = soup.find('p', class_='price')
...
bot = telebot.TeleBot(
    '5604399517:AAE1l4-SjlgyDbWCy8MV3Sg3WosgVApe_qE', parse_mode="None")


@bot.message_handler(commands=['help', 'start'])
def enviar(message):
    bot.reply_to(
        message, "Hola, soy un bot que1 te avisa cuando el precio de un producto cambia.\nPara usarme escribe /precio")


@bot.message_handler(func=lambda message: "/precio" in message.text)
def enviar_precio(message):
    bot.reply_to(message, "El precio actual es de " +
                 str(precioSuspFinal) + "€")
    if precioSuspFinal <= precioDeseado:
        bot.reply_to(
            message, f"¡ATENCION! Hay oferta,bajó el precio! Está en: {str(precioSuspFinal) + '€'}\nEnlace: https://cutt.ly/fXzM2xi")
    else:
...
if precioSuspFinal <= precioDeseado:
    bot.send_message(
        1325259916, f"¡ATENCION! Hay oferta,bajó el precio! Está en: {str(precioSuspFinal) + '€'}\nEnlace: https://cutt.ly/fXzM2xi")
